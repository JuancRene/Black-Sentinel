...
export function ServicesComparison() {
    ...
    if (value === true) {
      return (
        <div className="flex justify-center">
          <div className={`rounded-full p-1 ${isFeatured ? "bg-[#0AB9C3]/20" : isSpecial ? "bg-white/20" : "bg-zinc-800"}`}>
            <Check className={`w-4 h-4 ${isFeatured ? "text-[#0AB9C3]" : isSpecial ? "text-white" : "text-zinc-400"}`} />
          </div>
        </div>
      )
    }
  ...
  return (
    ...
    >
      ...
      {/* Glow ambiental */}
      <div className="absolute top-1/4 left-0 w-[600px] h-[600px] bg-[#0AB9C3]/5 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ${revealed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[#0AB9C3]/30 bg-[#0AB9C3]/5 backdrop-blur-md shadow-[0_0_15px_-3px_rgba(10,185,195,0.2)] mb-6">
            ...
            <span className="text-xs font-semibold text-[#0AB9C3] uppercase tracking-wider">Detalle Técnico</span>
          </div>
          
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-6">
            Comparativa <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0AB9C3] to-teal-500">Punto por Punto</span>
          </h2>
              ...
              <tr>
                ...
                {plans.map((plan) => (
                  <th 
                    ...
                    } ${plan.featured ? "bg-[#0AB9C3]/5" : ""}`}
                    onMouseEnter={() => setHoveredCol(plan.key)}
                    onMouseLeave={() => setHoveredCol(null)}
                  >
                    {plan.featured && (
                         <div className="absolute top-0 left-0 right-0 h-1 bg-[#0AB9C3] shadow-[0_0_10px_#0AB9C3]" />
                    )}
                    <div className="flex flex-col items-center gap-2">
                        <plan.icon className={`w-6 h-6 ${
                            plan.featured ? "text-[#0AB9C3]" : plan.isSpecial ? "text-white" : "text-zinc-500"
                        }`} />
                        <div>
                            <div className={`font-bold text-lg ${plan.featured ? "text-[#0AB9C3]" : "text-white"}`}>{plan.name}</div>
                  ...
                  <tr key={sIdx} className="bg-zinc-900/50">
                    <td colSpan={5} className="p-4 border-y border-zinc-800">
                      <div className="flex items-center gap-2 pl-2">
                        <section.icon className="w-4 h-4 text-[#0AB9C3]" />
                    ...
                    <button
                        ...
                        className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-all duration-300 ${
                            selectedPlanMobile === plan.key
                                ? plan.featured 
                                    ? "bg-[#0AB9C3] text-black border-[#0AB9C3]" 
                                    : plan.isSpecial
                     ...
                     <div key={plan.key} className={`${selectedPlanMobile === plan.key ? "block animate-fade-in" : "hidden"}`}>
                        ...
                        <div className="text-center mb-8">
                            <div className="inline-block p-3 rounded-2xl bg-zinc-900 mb-4">
                                <plan.icon className={`w-8 h-8 ${plan.featured ? "text-[#0AB9C3]" : "text-white"}`} />
                            </div>
                        ...
                        <div className="space-y-8">
                            {comparisonData.map((section, idx) => (
                                <div key={idx}>
                                    <h4 className="text-[#0AB9C3] text-xs font-bold uppercase tracking-widest mb-4 border-b border-zinc-800 pb-2">
                                        {section.category}
                                    </h4>
        ...
        <div className="mt-16 text-center">
             <Button
                size="lg"
                onClick={handleCTA}
                className="bg-transparent border border-[#0AB9C3] text-[#0AB9C3] hover:bg-[#0AB9C3] hover:text-black font-bold px-8 py-6 text-lg transition-all duration-300 hover:scale-105 rounded-xl"
             >
